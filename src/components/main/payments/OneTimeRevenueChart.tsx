...
const OneTimeRevenueChart: React.FC<Props> = ({ chargesByConnection, connections }) => {
    ...
    const chartData = useMemo(() => {
        ...
        // Filter one-time charges (successful one-time payments)
        const oneTimeCharges = allCharges.filter(charge =>
            charge.type === 'one-time' &&
            charge.status === 'successful'
        )

        if (oneTimeCharges.length === 0) return []

        // Check if all charges are within a week
        const sortedCharges = [...oneTimeCharges].sort((a, b) =>
            ...
            <CardContent>
                {chartData.length === 0 ? (
                    <div className="h-80 flex items-center justify-center text-muted-foreground">
                        No one-time revenue data available
                    </div>
                ) : (
...
export default OneTimeRevenueChart
